...
let databaseName =
  Meteor.settings.public.collections.candidates || 'candidates';
export const CandidatesDB = new Mongo.Collection(databaseName, {
  idGeneration: 'MONGO'
});
...
if (Meteor.isServer) {
  ...
  functions[CandidatesAddInfo] = function(contact, info, value) {
    try {
      ...
      check(info, String);
      check(value, String);
      return CandidateManager.updateCandidateSelectedInfo(
        contact,
        info,
        value,
        this.userId
  ...
  functions[CandidatesAddFileStats] = function(contact, info, value) {
    try {
      ...
      return CandidateManager.updateCandidateSelectedInfo(
        ...
        info,
        value,
        `${Meteor.user().username} attached a file / notes to ${info_trim}`,
        this.userId
      );
    } catch (err) {
      console.error(err);
  ...
  functions[CandidatesRemoveFileStats] = function(contact, info) {
    try {
      ...
      check(info, String);
      let info_trim = info.replace(/_notes|_file/gi, '');
      return CandidateManager.updateCandidateSelectedRemoveFile(
        contact,
        info,
        `${Meteor.user().username} removed a file to ${info_trim}`,
        this.userId
      );
    } catch (err) {
      console.error(err);
  ...
  Meteor.publish(CandidatesPub, function() {
    try {
      ...
      if (
        !isPermitted(user.profile.role, ROLES.ADMIN) &&
        !isPermitted(user.profile.role, ROLES.SUPERUSER)
      )
        query['category'] = { $ne: 'SPAMMER' };
      let cursor = CandidatesDB.find(query, {
      ...
      Util.setupHandler(this, databaseName, cursor, doc => {
        ...
        newDoc.joinedDt = moment(
          doc.joinedDate ? doc.joinedDate : doc.createdAt
        )
          .tz(timezone)
          .format('YYYY-MM-DD');
        if (doc.status) {
        ...
        } else newDoc.friendlyStatus = 'N/A Status';
        return newDoc;
      });
    } catch (err) {
      console.error(err);
  ...
  Meteor.publish(ValidCandidates, function(candidate) {
    try {
      ...
      if (candidate.filter.indexOf(SEARCH.CONTEXT) > -1)
        or.push({
          'lastMessage.text': { $regex: searchString, $options: 'i' }
        });
      if (candidate.filter.indexOf(SEARCH.TITLE) > -1)
        or.push({
          'lastMessage.subject': { $regex: searchString, $options: 'i' }
        });
      if (!(or.length && searchString.length)) or = [];
      ...
      if (candidate.filter.indexOf(SEARCH.STAFF_EMAIL) > -1) {
        ...
        if (user && user.profile.emails)
          user.profile.emails.forEach(email => {
            if (email.status === 'connected') emails.push(email.user);
          });
        var optRegexp = [];
      ...
      if (candidate.filter.indexOf(CANDIDATE_STATUS.JUNKS) < 0)
        query['category'] = { $ne: 'JUNKS' };
      ...
      if (candidate.filter.indexOf(SEARCH.values) > -1)
        query['values'] = { $gte: 10 };
      if (candidate.filter.indexOf(SEARCH.iq) > -1) query['iq'] = { $gte: 110 };
      ...
      let andQ = [];
      if (or.length) andQ.push({ $or: or });
      if (or1.length) andQ.push({ $or: or1 });
      if (or2.length) andQ.push({ $or: or2 });
      if (andQ.length > 1) query['$and'] = andQ;
      else if (or.length) query['$or'] = or;
      else if (or1.length) query['$or'] = or1;
      else if (or2.length) query['$or'] = or2;
      let count = CandidatesDB.find(query, {
  ...
  Meteor.publish(MessagesUnreadCountPub, function() {
    Counts.publish(
      this,
      MessagesUnreadCountPub,
      CandidatesDB.find({ retired: VALUE.FALSE, 'lastMessage.read': false })
    );
    Counts.publish(
      this,
      MessagesUnreadCountPub + '_claimed',
      CandidatesDB.find({
        retired: VALUE.FALSE,
    ...
    Counts.publish(
      this,
      MessagesUnreadCountPub + '_unclaimed',
      CandidatesDB.find({
        retired: VALUE.FALSE,
        'lastMessage.read': false,
