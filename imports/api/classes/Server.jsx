...
export default class Server {
  ...
  initListener() {
    ...
    Meteor.users
      .find({ $or: [{ retired: { $exists: false } }, { retired: 0 }] })
      .fetch()
      .forEach(user => {
  ...
  removeSender(id, email) {
    ...
    let connections = this.messageSender.filter(sender => {
      if (sender.id == id && sender.email === email) connection = sender;
  ...
  updateLastDateRetrieved() {
    let time = moment()
      .utc()
      .valueOf();
    let task = new ScheduledTask('at 00:01 am', () => {
      Meteor.users
        .find({ $or: [{ retired: { $exists: false } }, { retired: 0 }] })
        .fetch()
        .forEach(user => {
        ...
        .forEach(user => {
          if (!user.profile.drive) {
            let fileResource = Drive.newFolder(
              `${user.profile.first || user.username}-${user.profile.last}-${
                Meteor.settings.public.config.title
              }`,
            ...
            if (fileResource)
              Meteor.users.update(
                { _id: user._id },
                { $set: { 'profile.drive': fileResource.id } }
              );
          }
        });
  }
