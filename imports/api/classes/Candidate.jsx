...
class Candidate {
  ...
  isSpammerOrJunk() {
    return this.category === 'SPAMMER' || this.category === 'JUNKS';
  }
  ...
  isReply() {
    return (
      this.lastMessage.to === this.number ||
      this.lastMessage.to === this.email ||
      this.lastMessage.to === this.contact
    );
  }

  getSubject() {
    let subject = this.lastMessage.subject || this.lastMessage.html;
    subject = Util.trunc(subject ? subject : this.lastMessage.text, 22);
    if (subject) return subject;
  }

  getContact() {
    let space = 0;
    if (this.lastMessage.read) space += 3;
    if (!this.isReply()) space += 3;
    return Util.trunc(
      this.name || this.email || this.contact,
      (this.category ? 14 : 16) + space
    );
  }
  ...
  getCategory() {
    return (
      <div
        className="badge mr-1 text-light"
        style={{ backgroundColor: this.categoryColor }}
      >
        {this.category}
      </div>
    );
