...
class DropdownSelect extends React.Component {
    ...
    renderGroups() {
        return this.props.options.map((group, index) => {
            ...
            options.forEach((item) => {
                if (this.props.value && this.props.value.indexOf(item) > -1)
                    return;
                checked = false;
            });
            return (
                <div key={index}>
                    <a href='#' className={'list-group-item list-group-item-action pl-2'} key={index} onClick={() => {
                        this.change(options, !checked);
                    }}>
                        <input type='checkbox' checked={checked} readOnly /> <b>{group.label}</b>
                    </a>
    ...
    renderValue() {
        ...
        this.props.options.forEach((group) => {
            ...
            if (checked)
                ...
                value = this.renderValueOptions(group, value);
        });
        return value;
    }
    ...
    renderOptions(group) {
        return group.options.map((option, index) => {
            let checked = this.props.value && this.props.value.indexOf(option.value) > -1;
            return (
        ...
        return (
            <div className={this.props.className || ''}>
                <input
                    ...
                    className={'form-control dropdown-multi bg-light'}
