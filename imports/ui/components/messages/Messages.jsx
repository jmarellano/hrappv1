...
class Messages extends React.Component {
      ...
      displayOptions: [
          ...
          options: [
            { value: SEARCH.ASSIGNED, label: 'Assigned' },
            {
              ...
              element: (
                <span>
                  {'Claimed'}
                  <span className="pull-right mt-1 badge badge-warning">
                    {props.claimed}
                  </span>
                </span>
              )
        ...
        },
        {
          ...
          options: [
            ...
            { value: CANDIDATE_STATUS.JUNKS, label: 'JUNKS' }
          ]
        },
        {
          label: 'ALL TEXT',
          ...
          options: [
            {
              ...
              element: (
                <span>
                  {'resume'}
                  <span className="pull-right mt-1 badge badge-secondary">
                    {'> 5'}
                  </span>
                </span>
              )
  ...
  componentDidUpdate(prevProps) {
    if (
      prevProps.unclaimed !== this.props.unclaimed ||
      prevProps.claimed !== this.props.claimed
    ) {
      let displayOptions = this.state.displayOptions;
      displayOptions[1].options[1]['element'] = this.props.claimed ? (
        <span>
          {'Claimed'}
          <span className="pull-right mt-1 badge badge-warning">
            {this.props.claimed}
      ...
      displayOptions[1].options[3]['element'] = this.props.unclaimed ? (
        <span>
          {'Unclaimed'}
          <span className="pull-right mt-1 badge badge-warning">
  ...
  selectCandidate(candidate) {
    this.setState({
      candidate: this.props.candidates.filter(
...
export default withTracker(() => {
  ...
  return {
    notifications: NotificationsDB.find(
      {},
      { sort: { status: -1, createdAt: -1 } }
    )
      .fetch()
      .map(item => new Notif(item)),
    candidates: CandidatesDB.find({}, { sort: { 'lastMessage.createdAt': -1 } })
      .fetch()
